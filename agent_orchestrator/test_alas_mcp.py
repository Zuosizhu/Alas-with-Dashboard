...
def test_adb_screenshot(mock_ctx):
    result = alas_mcp_server.adb_screenshot.fn()
    assert result["content"][0]["type"] == "image"
    assert result["content"][0]["data"] == "base64data"

def test_adb_tap(mock_ctx):
    result = alas_mcp_server.adb_tap.fn(100, 200)
...
def test_adb_swipe(mock_ctx):
    result = alas_mcp_server.adb_swipe.fn(100, 100, 200, 200, 500)
...
def test_adb_swipe_uiautomator2_with_duration(mock_ctx):
    ...
    result = alas_mcp_server.adb_swipe.fn(50, 50, 150, 150, 300)
...
def test_adb_swipe_nemu_ipc_no_duration(mock_ctx):
    ...
    result = alas_mcp_server.adb_swipe.fn(75, 75, 175, 175, 400)
...
def test_alas_get_current_state(mock_ctx):
    result = alas_mcp_server.alas_get_current_state.fn()
...
def test_alas_goto_success(mock_ctx):
    ...
    result = alas_mcp_server.alas_goto.fn("page_main")
...
def test_alas_goto_invalid(mock_ctx):
    ...
    with pytest.raises(ValueError, match="unknown page"):
        alas_mcp_server.alas_goto.fn("invalid_page")

def test_alas_list_tools(mock_ctx):
    result = alas_mcp_server.alas_list_tools.fn()
    assert len(result) == 1
    assert result[0]["name"] == "test_tool"

def test_alas_call_tool(mock_ctx):
    mock_ctx._state_machine.call_tool.return_value = {"success": True}
    result = alas_mcp_server.alas_call_tool.fn("test_tool", {"arg": 1})
