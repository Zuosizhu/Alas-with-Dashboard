...
class Connection(ConnectionAttr):
        ...
        if port_serial and emu_serial:
            ...
            if port_device and emu_device:
                ...
                if port_device.status == "device" and emu_device.status == "offline":
                    ...
                    logger.info(
                        f"LDPlayer device pair found: {port_device}, {emu_device}. " f"Using serial: {self.serial}"
                    )
    ...
    def detect_package(self, set_config=True):
        ...
        if len(packages) == 0:
            logger.critical(
                f"No AzurLane package found, " f'please confirm AzurLane has been installed on device "{self.serial}"'
            )
            raise RequestHumanTakeover
        if len(packages) == 1:
