...
class Camera(MapOperation):
    ...
    def map_swipe(self, vector):
        ...
        Returns:
            bool: if camera moved.
        """
        logger.info(f"Map swipe: {str(vector)}")
    ...
    def _update_view(self):
        ...
        self._view_init()
        try:
            if not self.is_in_map() and not self.is_in_strategy_submarine_move() and not self.is_in_strategy_mob_move():
    ...
    def convert_global_to_local(self, location):
        ...
        logger.info(
            f"Global {location2node(location)} (camera={location2node(self.camera)}) -> Local {location2node(local)} (center={location2node(self.view.center_loca)})"
        )
    ...
    def convert_local_to_global(self, location):
        ...
        logger.info(
            f"Global {location2node(global_)} (camera={location2node(self.camera)}) <- Local {location2node(location)} (center={location2node(self.view.center_loca)})"
        )
