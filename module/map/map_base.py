...
class CampaignMap:
    ...
    def _find_path(self, location):
        ...
        if len(res) == 0:
            logger.warning(f"No path found. Destination: {str(location)}")
    ...
    def find_path(self, location, step=0, turning_optimize=False):
        ...
        if path is None or not len(path):
            ...
            return [location]
        logger.info("Full path: {}".format("[") + ", ".join([location2node(grid) for grid in path]) + "]")
        ...
        for start, end in zip(index[:-1], index[1:]):
            ...
            logger.info("Path: {}".format("[") + ", ".join([location2node(grid) for grid in local_path]) + "]")
        path = portal_path
