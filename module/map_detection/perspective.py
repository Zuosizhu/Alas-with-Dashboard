...
class Perspective:
    ...
    def load(self, image):
        ...
        logger.info(
            "{}s  {}   Horizontal: {} ({} inner, {} edge)".format(
                float2str(time_cost),
        ...
        logger.info(
            "Edges: {}{}{}    Vertical: {} ({} inner, {} edge)".format(
    ...
    def mid_cleanse(self, mids, is_horizontal, threshold=3):
        ...
        if diff > 0:
            logger.info(
                "{} coincident point unexpected: {}".format("Horizontal" if is_horizontal else "Vertical", str(coincident_point))
            )
