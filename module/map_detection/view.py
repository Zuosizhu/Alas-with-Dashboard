    ...
    def predict_swipe(self, prev, with_current_fleet=True, with_sea_grids=True):
        ...
        if with_current_fleet:
            ...
            if len(current_fleet) == 1 and len(previous_fleet) == 1:
                ...
                logger.info(
                    f'Map swipe predict: {diff} ({float2str(time.time() - start_time) + "s"}' f", current fleet match)"
                )
                return diff
        ...
        # Unable to predict
        logger.info(f"Map swipe predict: None " f'({float2str(time.time() - start_time) + "s"}, no match)')
        return None
