...
class OSMap(OSFleet, Map, GlobeCamera, StrategicSearchHandler):
    ...
    def handle_fog_block(self, repair=True):
        ...
        logger.warning(
            f"Triggered stuck fog status, restarting " f"game to resolve and continue " f"{self.config.task.command}"
        )
    ...
    def clear_question(self, drop=None):
        ...
        logger.warning(
            "Failed to goto question mark after 5 trail, " "this might be 2 adjacent fleet mechanism, stopped"
        )
        return False
