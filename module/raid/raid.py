...
def raid_ocr(raid, mode):
    ...
    if raid == "ESSEX":
        ...
        if server.server == "en":
            # Bold in EN
            return RaidCounter(button, letter=(148, 138, 123), threshold=80)
        if server.server == "jp":
            return RaidCounter(button, letter=(148, 138, 123), threshold=128)
        else:
            return DigitCounter(button, letter=(148, 138, 123), threshold=128)
    elif raid == "ALBION":
